...
/**
 * Fetch products with optional collection, tag, limit and after (cursor).
 * Returns { products: ShopifyProduct[], pageInfo: { hasNextPage, endCursor } }
 */
export async function fetchProducts({
  ...
  try {
    // Build query for either collectionByHandle(...) or products(...)
      ...
      const products = edges.map((e: any) => {
        ...
        return {
          ...
          images: (n.images?.edges || [])
            ...
            .filter(Boolean),
          price: n.variants?.edges?.[0]?.node?.price?.amount
